...
import { api, type BedPhotoReason, type BedPhotoResponse } from '../../api/client'
...
export function BedPhotosSection({
  ...
  const [lightbox, setLightbox] = useState<BedPhotoResponse | null>(null)
  const [confirmDelete, setConfirmDelete] = useState<number | null>(null)

  const { data: photos = [] } = useQuery({
    queryKey: ['bed-photos', bedId],
    queryFn: () => api.beds.photos(bedId),
  })

  const deleteMut = useMutation({
    mutationFn: (photoId: number) => api.beds.deletePhoto(bedId, photoId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['bed-photos', bedId] })
...
function AddBedPhotoDialog({
  ...
  const addMut = useMutation({
    mutationFn: () => {
      ...
      return api.beds.addPhoto(bedId, {
