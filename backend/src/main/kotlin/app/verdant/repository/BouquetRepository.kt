...
class BouquetRepository(private val ds: AgroalDataSource) {
    ...
    fun persist(bouquet: Bouquet): Bouquet {
        ds.connection.use { conn ->
            conn.prepareStatement(
                """INSERT INTO bouquet (org_id, source_recipe_id, name, description, image_url, price_sek, assembled_at, notes, created_at, updated_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, now(), now())""",
                ...
                ps.setObject(6, bouquet.priceSek)
            ...
            conn.prepareStatement(
                """UPDATE bouquet
                   SET source_recipe_id = ?, name = ?, description = ?, image_url = ?,
                       price_sek = ?, assembled_at = ?, notes = ?, updated_at = now()
                   WHERE id = ?"""
            ).use { ps ->
                ...
                ps.setObject(5, bouquet.priceSek)
