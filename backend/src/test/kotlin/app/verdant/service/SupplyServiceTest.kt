...
class SupplyServiceTest {
    ...
    @Test
    fun `seedInexhaustibleFertilizers persists five FERTILIZER rows for the given org`() {
        ...
        verify(typeRepo, times(5)).persist(captor.capture())
        val seeded = captor.allValues
        assertEquals(setOf("Hästgödsel", "Hönsgödsel", "Kompost", "Träaska", "Gräsklipp"), seeded.map { it.name }.toSet())
        seeded.forEach { row ->
        ...
        assertEquals("""{"npk":"0.6-0.2-0.5"}""", byName.getValue("Gräsklipp").properties)
    }
}
